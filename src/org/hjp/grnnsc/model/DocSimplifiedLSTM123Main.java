...
import org.hjp.grnnsc.data.Data;
import org.hjp.grnnsc.data.Funcs;
import org.hjp.grnnsc.nn.LinearLayer;
import org.hjp.grnnsc.nn.LookupLinearTanh;
import org.hjp.grnnsc.nn.SimplifiedLSTMLayer;
import org.hjp.grnnsc.nn.SoftmaxLayer;
import org.hjp.grnnsc.util.Metric;
...
public class DocSimplifiedLSTM123Main {
	...
	public DocSimplifiedLSTM123Main(String embeddingFileWord, int embeddingLengthWord, int windowSizeWordLookup1,
			...
			String trainFile, String testFile, double randomizeBase) throws Exception {
		...
		xseedLLT1 = new LookupLinearTanh(windowSizeWordLookup1, wordVocab.size(), outputLengthWordLookup,
				embeddingLengthWord);
		xseedLLT1.lookup.setEmbeddings(table);

		xseedLLT2 = new LookupLinearTanh(windowSizeWordLookup2, wordVocab.size(), outputLengthWordLookup,
				embeddingLengthWord);
		xseedLLT2.lookup.setEmbeddings(table);

		xseedLLT3 = new LookupLinearTanh(windowSizeWordLookup3, wordVocab.size(), outputLengthWordLookup,
				embeddingLengthWord);
	...
	public void run(int roundNum, double probThreshould, double learningRate, int classNum) throws Exception {
		...
		for (int round = 1; round <= roundNum; round++) {
			...
			for (int idxData = 0; idxData < trainDataList.size(); idxData++) {
				...
				DocSimplifiedLSTM docSimplifiedLSTM = new DocSimplifiedLSTM(xseedLLT1, xseedLLT2, xseedLLT3,
						wordIdMatrix, seedSimplifiedLSTM);
				...
				// we minus 1 because the saved goldRating is in range 1~5,
				// while what we need is in range 0~4
				...
				if (idxData % 1000 == 0) {
					System.out.println("running idxData = " + idxData + "/" + trainDataList.size() + "\t "
							...
							+ String.format("%.4f", lossV / lossC) + "\t" + new Date().toLocaleString());
				}
			}
	...
	public void predict(int round) throws Exception {
		...
		for (int idxData = 0; idxData < testDataList.size(); idxData++) {
			...
			DocSimplifiedLSTM docSimplifiedLSTM = new DocSimplifiedLSTM(xseedLLT1, xseedLLT2, xseedLLT3, wordIdMatrix,
					seedSimplifiedLSTM);
...
//		args[1] = "-embeddingLength 200";
//		args[1] = "-embeddingFile /Users/hjp/Workshop/Model/grnnsc/data/vec.bin";
//		args[2] = "-windowSizeWordLookup1 1";
//		args[3] = "-windowSizeWordLookup2 2";
//		args[4] = "-windowSizeWordLookup3 3";
//		args[5] = "-outputLengthWordLookup 50";
//		args[6] = "-classNum 10";
//		args[7] = "-roundNum 70";
//		args[8] = "-probThreshold 0.001";
//		args[9] = "-learningRate 0.03";
//		args[10] = "-randomizeBase -0.01";
//		args[11] = "-trainFile /Users/hjp/Workshop/Model/grnnsc/train/yelp-2013-train.txt.ss";
//		args[12] = "-testFile /Users/hjp/Workshop/Model/grnnsc/test/yelp-2013-test.txt.ss";
		...
		DocSimplifiedLSTM123Main main = new DocSimplifiedLSTM123Main(embeddingFile, embeddingLength,
				...
				trainFile, testFile, randomizeBase);
